...
var shuduSTDLayer = cc.Layer.extend({
    Sense:null,
    ctor:function () {
        //////////////////////////////
        // 1. super init first
...
/*** 不显示关卡**/
        ...
        */
        ...
        this.topList.attr({x: size.width - 180,	y: size.height -60,scale : shudu.UI.scale,visible:false,});
        
        //扫描
        btn_scan = new cc.MenuItemImage(//3X3
        		res.sd_scan,
        		res.sd_scans,
        		function (btn) {
        			that.btnPubCallBack(btn);
        ...
        btn_scan.attr({x: size.width - 150,	y: size.height -65,scale : shudu.UI.scale,});

        var menu = new cc.Menu(backBtn,rightBtn,this.topList,btn_scan);
        menu.x = 0;
        menu.y = 0;
        ...
        if(shudu.gameType == 4 || shudu.gameType == 6) {
            ...
            this.btnOpen.attr({x:this.MainNode.width / 4,y:80,scale : shudu.UI.scale,visible:false});
            ...
            //剩余翻开机会
            var openNum = new cc.LabelTTF(shudu.QOpened, 'Times New Roman', 52, cc.size(70, 70), cc.TEXT_ALIGNMENT_CENTER,0,cc.VERTICAL_TEXT_ALIGNMENT_CENTER);
        ...
        //看答案
        var btnShowAnswer = new cc.MenuItemImage(
        		res.sd_btn_answer,
        		res.sd_btn_answers,
        		function (btn) {
        			
        			if(shudu.QRstr == '')return false;
        			...
        			//显示答案
        			for(var i = 0; i<this.Qt[1].length; i++){
        				
        					var p = this.moveChess(parseInt(this.Qt[1][i])-1,i);

        			}
        			shudu.showAnswered = true;
        ...
        btnShowAnswer.attr({x:this.MainNode.width / 4,y:80,scale : shudu.UI.scale});

        bottomBtn.addChild(btnShowAnswer, 0);
        ...
        //用时
        var useTimeTxt = new cc.LabelTTF('该局用时：1000秒',  '黑体', 70, cc.size(620,90), cc.TEXT_ALIGNMENT_CENTER);
        useTimeTxt.setFontFillColor(cc.color('#ffffff'));
        useTimeTxt.attr({
        	x: jiesuan.width /2,
        	y: 350,
        });
    ...
    newGame:function(type,QtId,str){
        ...
        for(var i = 0; i<this.Qt[2].length; i++){
        	if(this.Qt[2][i] == '1'){
        		...
        		shudu.gameData[i]['chess']['chenge'] = false;
        		this.showDisMove(p);
        	}
        }
    ...
    replay:function(){
    	...
    	this.newGame(shudu.gameType, 0, shudu.QRstr);
    },
    
    _submit:function(){
    	
    	if(shudu.showAnswered)return false;
    ...
    Qt6X6:function(id ,str){
    	
    	key = id;
    	//cc.log(Qt6_data[key])
    	var Qt6 = unpackflm( str );
    	
    	if(!Qt6){
    		this.showAlertX('数据错误，请重试！',QRscan);
    		return false;
    	}
    	
    	
    	return [key , Qt6.answer , Qt6.flag];	//answer 答案   flag题面
    },
...
var shuduSTDLayerObj = 0;

var shuduGameScene2 = cc.Scene.extend({
    onEnter:function () {
        this._super();
        ...
        shudu.runtime.QRstr = '';
        shudu.QRstr = '';
        shudu.showAnswered = false;	//是否查看过答案
        
        shudu.QRstatus = 0; //扫描状态  0未扫码  1扫描中  2扫码完成（回调）
        
        
        shuduSTDLayerObj = new shuduSTDLayer();
        this.addChild(shuduSTDLayerObj);
...
cc.eventManager.addCustomListener('afterQRscan', function(event){ 
	cc.log("afterQRscan"); 
	clearListenQRstr();
	shudu.runtime.QRstr = shudu.QRstr;
	shuduSTDLayerObj.newGame(shudu.gameType, 0, event._userData.QRstr);
...
if(typeof shudu.listenQRstr != 'undefined')
	clearInterval(shudu.listenQRstr);
